...
%token tWINDOW tDOT tLINE tCIRCLE tCLEAR tFILL tPRINTER tSETUP
%token tBUTTON tALERT tWINDOW tMENU tCHECKBOX tRADIOBUTTON tTEXTCONTROL
...
%token tDRAW tTEXT tRECT tFLUSH tELLIPSE tSAVE
%token tRECT tGETCHAR tPUTCHAR tNEW tCURVE
...
%token tMESSAGE tIMAGE tSVG tTRANSLATE tGET tMOUSE tISMOUSEIN tMOUSEMOVE
...
/*  | tOPEN tWINDOW expression ',' expression {create_openwin(FALSE);} */
  ...
  */
  ...
  | tTEXTCONTROL tCLEAR string_expression {add_command(cTEXTCONTROL4,NULL);}
...
string_function: tLEFT '(' string_expression ',' expression ')' {create_function(fLEFT);}
  ...
  | tMESSAGE '(' ')' {create_function(fMESSAGE);}
  | tMOUSEMOVE {create_function(fMOUSEMOVE);}
  | tMOUSEMOVE '(' ')' {create_function(fMOUSEMOVE);}  
