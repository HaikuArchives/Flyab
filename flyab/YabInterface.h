...
#define YABINTERFACE_H
	...
	public:
		...
		// BHandler *ResolveSpecifier(BMessage *msg, int index, BMessage *spec, int form, const char *prop);
		void Run();
		...
		// void MessageCallback(Fl_Widget *widget);
//		void MessageCallback(Fl_Widget *widget);
		static void StaticMessageCallback(Fl_Widget *widget, void *yab);
		BPoint GetWindowCoordinates(Fl_Widget *parent, double x, double y);
		bool QuitRequested();
		bool ExitRequested();
		...
		const char* FilePanel(const char* mode, const char* title, const char* directory, const char*filename);
		...
		void FileBoxAdd(const char* id, const char* name, int pos, double maxWidth, double minWidth, double width, const char* option);
	...
	private:
		...
		pthread_t myThread;
		std::vector<YabView*> yabViewList;
		std::vector<char*> tooltips;
